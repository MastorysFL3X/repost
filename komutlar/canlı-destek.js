...
const emoji = message.client.emojis.get('');
      ...
      let destekKanal = "561452235926536192";
    ...
    collector.on('end', (collected, reason) => {
      ...
      if (reason === 'success') {
        ...
        client.channels.get(destekKanal).send('**Canlı Destek Talebini İptal Etmek İçin ``iptal`` Yazınız.**')
        ...
        client.on('message', message => {
          function contact() {
            ...
            if (message.channel.id === destekKanal) chan.send(`**Yetkili :**   ${message.content}`)
          }
          contact(client)
        })
      }
