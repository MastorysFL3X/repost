exports.run = async (client, msg, args) => {
 ...
 description: (':no_entry_sign: Ya geçerli birini etiketle ya da sana sövmemi istiyosan kendini etiketle.')
  ...
  if(member.id === "525521425495490571")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`:no_entry_sign: Hoop! Yapımcıma Laf Sokamazsın !!`)
}}) 
   if(member.id === "474517276297789460")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`:no_entry_sign: Yardımcı Laf Sokamazsın !!`)
}}) 
  if(member.id === "456083645015392257")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`:no_entry_sign: Hoop! Yapımcıma Laf Sokamazsın !!`)
}}) 
